...
float fund = 0.2; // the speed of the central sine
...
void draw()
{
  ...
  for (i = 0; i<sines.length; i++) // go through all the sines
  {
    ...
    // setup tracing
    if (trace) {
      stroke(75, 0, 130*(float(i)/sines.length), alpha); // blue
      fill(75, 0, 130, alpha/2); // also, um, blue
  ...
  for (i = 0; i<sines.length; i++) // go through all the sines
  {
    ...
    // setup tracing
    if (trace) {
      stroke(189,252*(float(i)/sines.length), 201, alpha); // blue
      fill(189, 252, 201, alpha/2); // also, um, blue
      erad = 5.0*(1.0-float(i)/sines.length); // pen width will be related to which sine
    }
    float radius = rad/(i+1); // radius for circle itself
    rotateZ(sines[i]); // rotate circle
  ...
  pushMatrix();
  translate((3*width)/4, height/2); // move to middle of screen
  for (i = 0; i<sines.length; i++) // go through all the sines
  {
    float erad = 0; // radius for small "point" within circle... this is the 'pen' when tracing
    // setup tracing
    if (trace) {
      stroke(250*(float(i)/sines.length),128, 114,  alpha); // blue
      fill(250, 128, 114, alpha/2); // also, um, blue
      erad = 5.0*(1.0-float(i)/sines.length); // pen width will be related to which sine
    }
    float radius = rad/(i+1); // radius for circle itself
    rotateZ(sines[i]); // rotate circle
